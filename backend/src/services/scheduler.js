    ...
    cron.schedule('* * * * *', async () => {
        ...
        try {
            ...
            for (const doc of doctors) {
                    ...
                    if (hasScheduleToday) {
                        quota = await prisma.dailyQuota.create({
                            data: {
                                ...
                                status: 'CLOSED' // Start closed, let logic open it
                            }
                        });
                ...
                if (quota) {
                    // 2. Check Schedule for Status
                    const schedulesToday = doc.schedules.filter(s => s.day === dbDay);
                    let shouldBeOpen = false;

                    for (const schedule of schedulesToday) {
                        // Handle multiple time ranges like "07.00 - 10.00 & 14.00 - 17.00"
                        const ranges = schedule.time.split('&');
                        for (const range of ranges) {
                            if (isWithinTime(range)) {
                                shouldBeOpen = true;
                                break;
                            }
                        }
                        if (shouldBeOpen) break;
                    }

                    // 3. Update Status if changed
                    // Only update if current status is OPEN or CLOSED (don't override BREAK if manually set? 
                    // For "serba otomatis", we override everything based on schedule).

                    const newStatus = shouldBeOpen ? 'OPEN' : 'CLOSED';

                    if (quota.status !== newStatus && quota.status !== 'BREAK') {
                        // Optional: Allow manual BREAK to persist? 
                        // User asked for "live status buka tutup... mengikuti jadwal".
                        // Let's strictly follow schedule for OPEN/CLOSED. 
                        // If doctor is on BREAK manually, we might want to respect that, 
                        // but if schedule says CLOSED, we should close.

                        // Logic:
                        // If Schedule says OPEN:
                        //    - If currently CLOSED -> set OPEN
                        //    - If currently BREAK -> keep BREAK (manual override)
                        //    - If currently OPEN -> keep OPEN
                        // If Schedule says CLOSED:
                        //    - Always set CLOSED (Force close)

                        let finalStatus = quota.status;
                        ...
                        if (finalStatus !== quota.status) {
                            ...
                            console.log(`[Scheduler] Updated ${doc.name} to ${finalStatus}`);
                        }
                    }
                }
            }
