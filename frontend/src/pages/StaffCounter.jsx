...
const StaffCounter = () => {
    ...
    if (!isInitialized) {
        return (
            <div className="min-h-screen bg-theme-bg flex items-center justify-center p-4 font-sans text-theme-text transition-colors duration-300">
                <div className="bg-modern-card rounded-3xl shadow-2xl p-8 w-full max-w-md border border-white/50 dark:border-white/10 backdrop-blur-xl">
                    ...
                    <form onSubmit={handleInitSubmit} className="space-y-6">
                        ...
                        <div className="bg-salm-light-blue/20 p-5 rounded-2xl border border-salm-light-blue/30 mx-auto">
                            ...
                            <select
                                required
                                className="w-full bg-white/80 dark:bg-gray-800 border border-salm-light-blue/50 text-gray-700 dark:text-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-salm-blue/30 outline-none backdrop-blur-sm transition-all hover:bg-white dark:hover:bg-gray-700"
                        ...
                        <div>
                            ...
                            <select required className="w-full bg-theme-bg border border-theme-border text-gray-800 dark:text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-salm-blue/30 outline-none" value={config.counterName} onChange={(e) => setConfig({ ...config, counterName: e.target.value })}>
                        ...
                        <div>
                            ...
                            <select className="w-full bg-theme-bg border border-theme-border text-gray-800 dark:text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-salm-blue/30 outline-none" value={config.poliId} onChange={(e) => setConfig({ ...config, poliId: e.target.value })}>
    ...
    return (
        <div className="min-h-screen bg-theme-bg flex flex-col md:flex-row text-theme-text font-sans transition-colors duration-300">
            <Toaster position="top-right" />

            {/* Sidebar */}
            <div className="w-full md:w-80 bg-modern-card border-r border-gray-100 dark:border-white/5 flex flex-col h-screen shadow-sm z-10 glass">
                <div className="p-6 border-b border-gray-100/50 dark:border-white/5">
                    ...
                    <select className="w-full text-sm border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-salm-blue/20 outline-none bg-gray-50/50 dark:bg-gray-800/50 text-gray-700 dark:text-gray-200 mb-3" value={config.poliId} onChange={(e) => { const newConfig = { ...config, poliId: e.target.value }; setConfig(newConfig); localStorage.setItem('staffCounterConfig', JSON.stringify(newConfig)); }}>
                        <option value="all">Semua Poli</option>
                        {polies.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {waitingList.map((queue) => (
                        <div key={queue.id} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-salm-light-blue shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start pl-2">
                                <div>
                                    <span className="text-2xl font-bold block text-gray-800 dark:text-white group-hover:text-salm-blue transition-colors tracking-tight">{queue.queue_code}</span>
                ...
                <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
            ...
            {showSkippedModal && (
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md flex items-center justify-center z-50 animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-8 w-full max-w-lg max-h-[80vh] flex flex-col shadow-2xl border border-white/20 dark:border-gray-700">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Skipped Patients</h2>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
                            {skippedList.map((queue) => (
                                <div key={queue.id} className="bg-gray-50 dark:bg-gray-700/50 p-5 rounded-2xl border border-gray-100 dark:border-gray-600 flex justify-between items-center group hover:bg-white dark:hover:bg-gray-700 hover:shadow-md transition-all">
                                    <div>
                                        <span className="text-2xl font-bold text-gray-800 dark:text-white block group-hover:text-salm-blue transition-colors">{queue.queue_code}</span>
