        ...
        <PageWrapper title="Radiology Unit">
            ...
            <div className="relative min-h-screen p-8 max-w-[1600px] mx-auto z-10 font-sans">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-6">
                    <ModernHeader
                        title="Radiologi"
                        subtitle="Medical Imaging Center"
                ...
                <div className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-2xl p-2 rounded-[24px] border border-white/20 dark:border-gray-700 flex flex-col md:flex-row gap-4 mb-4 shadow-sm">
                        ...
                        {['PENDING', 'COMPLETED'].map((tab) => (
                            <button
                                ...
                                className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${activeTab === tab
                                    ? 'bg-white dark:bg-gray-800 text-indigo-600 shadow-sm scale-100'
                                    : 'text-gray-500 hover:text-indigo-600 hover:bg-white/50'
                                    }`}
                            >
                                {tab === 'PENDING' ? 'Queue' : 'Archive'}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 relative group">
                        ...
                        <input
                            type="text"
                            placeholder="Search by Patient Name, MR Number, or Order Type..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full h-full bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm pl-12 pr-4 rounded-xl border-2 border-transparent focus:border-indigo-500/50 focus:bg-white dark:focus:bg-gray-800 transition-all outline-none font-medium placeholder:text-gray-400"
                        />
                    </div>
                </div>
                ...
                <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-3xl rounded-[32px] border border-white/40 dark:border-gray-700 shadow-xl overflow-hidden min-h-[500px] flex flex-col">
                    ...
                    <div className="grid grid-cols-12 gap-4 px-8 py-5 border-b border-gray-200/50 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30 text-xs font-bold text-gray-400 uppercase tracking-widest">
                        ...
                        <div className="col-span-4">Scan Type / Notes</div>
                        ...
                        <AnimatePresence mode="popLayout">
                            {filteredOrders.length === 0 ? (
                                ...
                                >
                                    ...
                                    <p className="text-lg font-medium text-gray-500">No matching scans found.</p>
                                </motion.div>
                            ) : (
                                filteredOrders.map((order, index) => (
                                    ...
                                    >
                                        <div className="col-span-1 text-center font-bold text-gray-300 group-hover:text-indigo-500 transition-colors">
                                            #{index + 1}
                                        </div>
                                        ...
                                        <div className="col-span-4">
                                            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 line-clamp-2 leading-relaxed flex items-center gap-2">
                                                <Activity size={16} className="text-indigo-400" />
                                                {order.notes}
                                            </p>
                                            {order.status === 'COMPLETED' && (
                                                <p className="text-xs text-green-600 mt-1 font-mono flex items-center gap-1 pl-6">
                                                    <CheckCircle size={10} /> Report Finalized
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="col-span-2 flex justify-center">
                                            {order.status === 'PENDING' ? (
                                                <button
                                                    ...
                                                    className="opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 py-2.5 px-6 rounded-xl font-bold text-white shadow-lg shadow-indigo-500/20 active:scale-95 flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-sm"
                                                >
                                                    Start <Zap size={16} fill="currentColor" />
                                                </button>
                                            ) : (
